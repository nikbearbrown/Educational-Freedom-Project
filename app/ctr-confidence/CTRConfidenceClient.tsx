...
function inverseNormalCdf(p: number) {
  // Acklam approximation, good enough for UI
  ...
  let q,r;
  if (pp < plow) { q = Math.sqrt(-2 * Math.log(pp));
    return (((((c1*q + c2)*q + c3)*q + c4)*q + c5)*q + c6) / ((((d1*q + d2)*q + d3)*q + d4));
  } else if (pp <= phigh) { q = pp - 0.5; r = q*q;
    return (((((a1*r + a2)*r + a3)*r + a4)*r + a5)*r + a6) * q /
           (((((b1*r + b2)*r + b3)*r + b4)*r + b5) + 1);
  } else { q = Math.sqrt(-2 * Math.log(1 - pp));
    return -(((((c1*q + c2)*q + c3)*q + c4)*q + c5)*q + c6) / ((((d1*q + d2)*q + d3)*q + d4));
  }
}
...
export default function CTRConfidenceClient() {
  ...
  return (
    <Suspense fallback={<div className="p-4">Loading math…</div>}>
      <MathJaxContext version={3} config={mjConfig}>
        <div className="container px-4 md:px-6 mx-auto py-12">
          <div className="max-w-4xl mx-auto">
            <div className="mb-12">
              <h1 className="text-4xl md:text-5xl font-bold mb-4">CTR Bounds & Confidence Intervals (Interactive)</h1>
            ...
            <section className="mb-10">
              ...
              <div className="prose prose-lg dark:prose-invert max-w-none">
                <p>
                  Treat clicks like coin flips. With few flips, low heads can be bad luck. A
                  <strong> confidence interval</strong> gives a safe range for the true CTR.
                  If the <em>upper</em> end of that range is still below your target, it’s fair to remove the keyword.
                </p>
